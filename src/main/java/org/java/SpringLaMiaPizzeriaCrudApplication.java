...
public class SpringLaMiaPizzeriaCrudApplication {
	...
	@Bean
	public CommandLineRunner loadData(PizzaRepository pizzaRepository) {
		...
		return (args) -> {
			pizzaRepository.save(new Pizza("Margherita", "Pomodoro, mozzarella, basilico", null, new BigDecimal("5.50")));
			pizzaRepository.save(new Pizza("Diavola", "Pomodoro, mozzarella, salame piccante", null, new BigDecimal("7.00")));
			pizzaRepository.save(new Pizza("Quattro Formaggi", "Mozzarella, gorgonzola, parmigiano, emmental", null, new BigDecimal("8.50")));
		};
	}
